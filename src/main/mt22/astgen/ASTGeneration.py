...
class ASTGeneration(MT22Visitor):
    ...
    def visitFuncdecl(self, ctx:MT22Parser.FuncdeclContext):
        inherit = None
        if ctx.INHERIT(): inherit = ctx.ID(1).getText()
    ...
    def visitExprlist(self, ctx:MT22Parser.ExprlistContext):
        if ctx.exprprime(): return self.visit(ctx.exprprime())
        return []
    ...
    def visitParamlist(self, ctx:MT22Parser.ParamlistContext):
        if ctx.paramprime(): return self.visit(ctx.paramprime())
        return []
    ...
    def visitIfstmt(self, ctx:MT22Parser.IfstmtContext):
        ...
        fstmt = None
        if ctx.elsestmt(): fstmt = self.visit(ctx.elsestmt())
    ...
    def visitReturnstmt(self, ctx:MT22Parser.ReturnstmtContext):
        expr = None
        if ctx.expr(): expr = self.visit(ctx.expr())
    ...
    def visitStmtlist(self, ctx:MT22Parser.StmtlistContext):
        if ctx.stmtlist():
            if ctx.stmt(): return [self.visit(ctx.stmt())] + self.visit(ctx.stmtlist())
    ...
    def visitExpr7(self, ctx:MT22Parser.Expr7Context):
        ...
        if ctx.BOOLEANLIT(): return BooleanLit(ctx.BOOLEANLIT().getText() == "true")
        if ctx.STRINGLIT(): return StringLit(ctx.STRINGLIT().getText())
        if ctx.funccall(): return self.visit(ctx.funccall())
        if ctx.arraycell(): return self.visit(ctx.arraycell())
    ...
    def visitReturnabletype(self, ctx:MT22Parser.ReturnabletypeContext):
        if ctx.array(): return self.visit(ctx.array())
    ...
    def visitIntprime(self, ctx:MT22Parser.IntprimeContext):
        if ctx.CM(): return [int(ctx.INTLIT().getText())] + self.visit(ctx.intprime())
